...
/**
 * This class is used to compare two XML {@code Xml} objects with each other.
 * User: msundell
 */
public class XmlReporter {
    /**
     * This method is used to compare two XML {@code Xml} objects with each other. The differences are written to
     ...
     * @return @{code Report} object
     */
    public Report compare(Xml a, Xml b, Arbitrator arbitrator, Report report) {
        // validations
        if(a == null) {
        ...
        if(report == null) {
            report = new DefaultReport(nameA, nameB);
        }

        // compare documents
        if (a.getRoot() != null || b.getRoot() != null) {
            if(a.getRoot() == null) {
                report.add(new Difference("", "", String.format("There's only data in: %s", b.getName())));
            } else if(b.getRoot() == null) {
                report.add(new Difference("", "", String.format("There's only data in: %s", a.getName())));
            } else {
